...
Micron(MU), Intel(INTC), SanDisk(SNDK), AST SpaceMobile(ASTS), Marvell(MRVL), Coherent(COHR) 등 반도체/기술주 관련 뉴스를 매일 자동으로 수집하고, Claude API로 요약·투자 심리 분석을 제공하는 Next.js 웹앱입니다.
...
- 종목별 URL(`/mu`, `/intc`, `/sndk`, `/asts`, `/mrvl`, `/cohr`)로 전환 가능한 6개 종목 뉴스 대시보드
- Google News, Yahoo Finance RSS에서 종목별 관련 뉴스를 최근 24~48시간 기준으로 수집 (최대 18건)
- Claude API로 전체 요약, 주요 이슈 3~5개, 긍정/중립/부정 톤 분석(0~100점), 주가 영향 요인을 한국어로 정리
- Next.js ISR(12시간 캐시) + Vercel Cron으로 DB 없이 하루 1회 6개 종목 전체 자동 갱신
- Claude API 호출이 실패해도 원본 뉴스 헤드라인은 항상 표시되는 graceful fallback (종목별 독립)
...
3. 수동으로 즉시 테스트하려면 배포된 URL에 직접 접속: `https://<배포URL>/api/refresh` (브라우저 또는 `curl`로 호출 시 `{"ok":true, "companies":[{"ticker":"MU","digestOk":true}, ...]}` 형태로 6개 종목 결과가 모두 포함된 응답이 오면 정상)
...
mu-news-dashboard/
├── app/
│   ├── page.tsx           # 루트 (/) → 기본 종목(MU)으로 리다이렉트
│   ├── [ticker]/page.tsx  # 실제 대시보드 템플릿 (ISR, revalidate 12시간, 6개 종목 정적 생성)
│   └── api/refresh/       # Vercel Cron이 호출하는 6개 종목 일괄 캐시 갱신 라우트
├── components/
│   └── CompanyNav.tsx      # 종목 전환 탭 (그 외 컴포넌트는 종목 무관 범용)
├── lib/
│   ├── rss.ts              # RSS 수집/파싱 (종목별 파라미터화)
│   ├── claude.ts            # Claude API 요약/분석 (종목별 파라미터화)
│   ├── digest.ts             # 캐싱 오케스트레이터 (종목별 unstable_cache 키/태그)
│   └── config.ts              # COMPANIES 레지스트리, 피드 URL, 모델명 등 설정값
└── vercel.json             # Vercel Cron 스케줄 설정
```

지원 종목: MU(Micron Technology), INTC(Intel), SNDK(SanDisk), ASTS(AST SpaceMobile), MRVL(Marvell Technology), COHR(Coherent Corp). `lib/config.ts`의 `COMPANIES` 배열에 항목을 추가/수정하면 종목을 늘리거나 바꿀 수 있습니다.
